...
export interface EvaluatedMove {
  ...
  san: string;
  fen: string;
  evalCentipawns: number; // Positive = White advantage, Negative = Black advantage
...
// Simple heuristic centipawn evaluator for chess board position
function evaluateBoard(chess: Chess): number {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      // Positional bonus: center control (squares d4, d5, e4, e5)
      let posBonus = 0;
      if ((r === 3 || r === 4) && (c === 3 || c === 4)) {
        posBonus = 20;
      }
...
export class ChessReviewAgent {
  /**
   * Reviews a complete game from list of SAN move strings
   */
  public static reviewGame(moveHistory: string[]): {
    ...
    for (let i = 0; i < moveHistory.length; i++) {
      ...
      // Check number of legal moves before making the move
      ...
      // For white: eval drop (prevEval - currentEval)
      // For black: eval increase (currentEval - prevEval)
      let cpl = color === 'w' ? prevEval - currentEval : currentEval - prevEval;
      if (cpl < 0) cpl = 0; // Move improved position
      ...
      if (i < 4) {
        ...
        // Check if brilliant (sacrificed material for massive positional/checkmate gain)
    ...
    // Accuracy formula: 100 - (100 * (1 - exp(-0.005 * CPL)))
